	...
	protected RenameResult ask_rename(File src_file, File dest_file, out File new_dest) {
		...
		if(ui != null) {
			try {
				...
				var src_info = src_file.query_info("standard::*,time::*", FileQueryInfoFlags.NOFOLLOW_SYMLINKS, cancellable);
				var dest_info = dest_file.query_info("standard::*,time::*", FileQueryInfoFlags.NOFOLLOW_SYMLINKS, cancellable);
